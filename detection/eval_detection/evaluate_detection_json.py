...
def load_gt_segmentations(annotations):
    gt_base = pd.DataFrame({
        ...
        'action': annotations['action_class'] if "action_class" in annotations else annotations['class_id'],
        'narration' : annotations.index
    })

    gt_base['label'] = annotations['action_class'] if "action_class" in annotations else annotations['class_id']
...
def load_predicted_segmentations(submission):
    ...
    for k, v in results.items():
        ...
        actions += [int(vv['action']) for vv in v]
        vids += [k] * len(v)
...
def validate_submission(
    ...
    def validate_task_entry(entry, task):
        if task in entry:
            ...
            if class_entry not in np.arange(num_actions):
...
def main(args):
    ...
    print(f"    Evaluating action...", end="")
    maps, avg = ANETdetection(groundtruth_df, submission, filename=args.path_to_json).evaluate()
    dump(display_metrics, maps, 'action')
    display_metrics[f"action_map_avg"] = avg*100
    print(f"        action evaluated")
