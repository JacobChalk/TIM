...
from .perception_record import PerceptionVideoRecord
...
class Perception(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        logger.info("Constructing Perception Test {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        for file in path_annotations_pickle:
            for ii, tup in enumerate(pd.read_pickle(file).iterrows()):
                for idx in range(self._num_clips):
                    record = PerceptionVideoRecord(tup)
        ...
        ), "Failed to load Perception test split {} from {}".format(
