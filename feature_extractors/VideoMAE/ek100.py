...
class EKRawFrameClsDataset(Dataset):
    ...
    def __init__(self,
        ...
        import pandas as pd
        df_dataset = pd.read_csv(self.anno_path)
        self.dataset_samples = [f"{self.data_path}/{row['video_id']}" for _, row in df_dataset.iterrows()]
        self.total_frames = [row['stop_frame'] - row['start_frame'] for _, row in df_dataset.iterrows()]
        self.frame_offsets = df_dataset['start_frame'].tolist()
        self.verb_array = df_dataset['verb_class'].tolist()
        self.noun_array = df_dataset['noun_class'].tolist()
        ...
        if (mode == 'train'):
            ...
            self.test_frame_offsets = []
            self.test_verb_array = []
            self.test_noun_array = []
            for ck in range(self.test_num_segment):
                for cp in range(self.test_num_crop):
                    for idx in range(len(self.dataset_samples)):
                        self.test_seg.append((ck, cp))
                        self.test_dataset.append(self.dataset_samples[idx])
                        self.test_total_frames.append(self.total_frames[idx])
                        self.test_frame_offsets.append(self.frame_offsets[idx])
                        self.test_verb_array.append(self.verb_array[idx])
                        self.test_noun_array.append(self.noun_array[idx])
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            frame_offset = self.frame_offsets[index]
            ...
            if args.num_sample > 1:
                ...
                for _ in range(args.num_sample):
                    ...
                    verb = self.verb_array[index]
                    noun = self.noun_array[index]
                    frame_list.append(new_frames)
                    verb_list.append(verb)
                    noun_list.append(noun)
                    index_list.append(index)
                return frame_list, (verb_list, noun_list), index_list, {}
            else:
                buffer = self._aug_frame(buffer, args)

            return buffer, (self.verb_array[index], self.noun_array[index]), index, {}

        elif self.mode == 'validation':
            sample = self.dataset_samples[index]
            total_frame = self.total_frames[index]
            frame_offset = self.frame_offsets[index]
            buffer = self.load_frame(sample, total_frame, frame_offset)
            if len(buffer) == 0:
                while len(buffer) == 0:
                    warnings.warn(
            ...
            frame_offset = self.test_frame_offsets[index]
            ...
            buffer = self.data_transform(buffer)
            return buffer, (self.test_verb_array[index], self.test_noun_array[index]), index, \
                   chunk_nb, split_nb
        else:
    ...
    def load_frame(self, sample, num_frames, frame_offset, sample_rate_scale=1):
        ...
        if self.mode == 'test':
            ...
            for idx in all_index:
                frame_fname = os.path.join(fname,
                                           self.filename_tmpl.format(idx + 1 + frame_offset))
