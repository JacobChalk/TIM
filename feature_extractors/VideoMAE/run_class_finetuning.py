...
def get_args():
    ...
    parser.add_argument('--data_set',
                        ...
                        choices=[
                            ...
                            'image_folder', 'ANet', 'MixKinetics', 'EK100'
                        ],
                        type=str,
...
def main(args, ds_init):
    ...
    if args.eval:
        ...
        if global_rank == 0:
            ...
            if len(args.nb_classes) == 2:
                final_top1_v, final_top5_v, final_top1_n, final_top5_n = merge(args.output_dir, num_tasks, multitask=True)
                print(
                    f"Accuracy of the network on the {len(dataset_test)} test videos: Verb Top-1: {final_top1_v:.2f}%, Verb Top-5: {final_top5_v:.2f}%, Noun Top-1: {final_top1_n:.2f}%, Noun Top-5: {final_top5_n:.2f}"
                )
                log_stats = {'Final verb top-1': final_top1_v, 'Final verb Top-5': final_top5_v, 'Final noun top-1': final_top1_n, 'Final noun Top-5': final_top5_n}
            else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats = validation_one_epoch(data_loader_val, model, device)
            if 'verb_acc1' in test_stats:
                print(
                    f"Verb/noun accuracy of the network on the {len(dataset_val)} val images: {test_stats['verb_acc1']:.2f}/{test_stats['noun_acc1']:.2f}%"
                )
            else:
                print(
                    f"Accuracy of the network on the {len(dataset_val)} val images: {test_stats['acc1']:.2f}%"
                )
            key_acc = 'noun_acc1' if 'noun_acc1' in test_stats else 'acc1'
            if max_accuracy < test_stats[key_acc]:
                max_accuracy = test_stats[key_acc]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(args=args,
                                     model=model,
                                     model_without_ddp=model_without_ddp,
            ...
            if log_writer is not None:
                if 'verb_acc1' in test_stats:
                    log_writer.update(val_acc1=test_stats['verb_acc1'],
                                    head="perf",
                                    step=epoch)
                    log_writer.update(val_acc5=test_stats['verb_acc5'],
                                    head="perf",
                                    step=epoch)
                    log_writer.update(val_acc1=test_stats['noun_acc1'],
                                    head="perf",
                                    step=epoch)
                    log_writer.update(val_acc5=test_stats['noun_acc5'],
                                    head="perf",
                                    step=epoch)
                    log_writer.update(val_loss=test_stats['loss'],
                                    head="perf",
                                    step=epoch)
                    log_writer.update(val_loss=test_stats['verb_loss'],
                                    head="perf",
                                    step=epoch)
                    log_writer.update(val_loss=test_stats['noun_loss'],
                                    head="perf",
                                    step=epoch)
                else:
