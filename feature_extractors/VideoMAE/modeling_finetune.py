...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if isinstance(num_classes, int):
            ...
            self.verb_head = nn.Linear(
                embed_dim, num_classes[0])
            self.noun_head = nn.Linear(
                embed_dim, num_classes[1])
        ...
        if isinstance(num_classes, int):
            ...
            self.verb_head.weight.data.mul_(init_scale)
            self.verb_head.bias.data.mul_(init_scale)
            self.noun_head.weight.data.mul_(init_scale)
            self.noun_head.bias.data.mul_(init_scale)
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        ...
        if isinstance(num_classes, int):
            ...
            self.verb_head = nn.Linear(
                self.embed_dim, num_classes[0])
            self.noun_head = nn.Linear(
                self.embed_dim, num_classes[1])
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'head'):
            ...
            x_v = self.verb_head(x)
            x_n = self.noun_head(x)
            if self.num_segment > 1 and self.training:
                x_v = x_v.view((-1, self.num_segment) + x_v.size()[1:])
                x_v = x_v.mean(dim=1)

                x_n = x_n.view((-1, self.num_segment) + x_n.size()[1:])
                x_n = x_n.mean(dim=1)
            x = (x_v, x_n)     
